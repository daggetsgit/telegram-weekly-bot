...
- Помни, что transcript автоматический. Критичные медицинские, финансовые и юридические детали помечай: “verify against original audio if critical”.
...
- Такие voice/audio отмечай как “requires manual review/transcription”.
...
Отдельно перечисли все решения, договорённости и подтверждения, которые были явно зафиксированы или вероятно подтверждены скриншотами/реакциями. Если подтверждение не стопроцентное, так и напиши.
...
9. Файлы и вложения
Не перечисляй все файлы механически. Раздели важные вложения на группы:
- важные для решений;
- документы/договоры/прайсы;
- технические или контекстные скриншоты.

Для каждого важного файла укажи:
- название файла;
- тип файла;
- к какой теме относится;
- что он подтверждает или почему важен;
...
Вынеси сюда:
- critical details from automatic transcripts;
- документы, которые нельзя прочитать;
- суммы/валюты/реквизиты со скриншотов, если они плохо читаются;
- неподтверждённые пересказы встреч;
- юридические/финансовые условия, требующие проверки.
